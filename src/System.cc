...
void System::Shutdown()
{
    ...
    const int MAX_WAIT_ITERATIONS = 2000;
    
    while(!mpLocalMapper->isFinished() || !mpLoopCloser->isFinished() || mpLoopCloser->isRunningGBA())
    {
        if(timeout_counter++ > MAX_WAIT_ITERATIONS)
        {
            cout << "WARNING: Shutdown timeout reached!" << endl;
            break;
        }
    ...
    {
        cout << "Joining LocalMapping thread..." << endl;
        mptLocalMapping->join();
    }
    if(mptLoopClosing && mptLoopClosing->joinable())
    {
        cout << "Joining LoopClosing thread..." << endl;
        mptLoopClosing->join();
    }
    if(mpViewer && mptViewer && mptViewer->joinable())
    {
        cout << "Joining Viewer thread..." << endl;
        mptViewer->join();
    }
    ...
    // COMMENT OUT THIS LINE - It causes segfault during destruction
    // if(mpViewer)
...
void System::SaveKeyFrameTrajectoryEuRoC(const string &filename)
{
    ...
    vector<Map*> vpMaps = mpAtlas->GetAllMaps();
    Map* pBiggerMap = nullptr;  // Initialize to nullptr
    int numMaxKFs = 0;
    
    for(Map* pMap : vpMaps)
    {
        if(pMap && pMap->GetAllKeyFrames().size() > numMaxKFs)  // Add null check
        {
            numMaxKFs = pMap->GetAllKeyFrames().size();
            pBiggerMap = pMap;
        }
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        // ADD ROBUST NULL AND BAD CHECKS
        if(!pKF || pKF->isBad())
            continue;
        ...
        try {  // Add exception handling
            ...
            cerr << "Exception while saving KeyFrame " << i << ": " << e.what() << endl;
            continue;
        }
    }
